...
class RolloutConfig(BaseConfig):
    ...
    mode: str = "async"
    chat_scheduler: Optional[str] = None
    ...
    def __post_init__(self):
        ...
        # Ascend SPMD rollout still uses sync mode for trainer-driven batch generation.
        if self.mode == "sync":
            warnings.warn(
                "Rollout mode 'sync' is kept for Ascend SPMD vLLM compatibility. "
                "Use async only for server-mode rollout.",
                DeprecationWarning,
                stacklevel=2,
            )
