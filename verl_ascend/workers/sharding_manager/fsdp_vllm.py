...
class FSDPVLLMShardingManager(BaseShardingManager):
                ...
                if self.layered_summon:
                    ...
                    with FSDP.summon_full_params(self.module, writeback=False):
                        if self.base_sync_done:
                            ...
                            orig_dev = "cpu" if "cpu" in next(model.parameters()).device.type else get_device_name()
                            model = model.to('cpu')
    ...
    def update_params(self, updated_params, peft_config=None):
        ...
        device = torch.device(f"{get_device_name()}:{get_device_id()}")  # used when fsdp2 set cpu_offload_policy
